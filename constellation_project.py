...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║                      🌌 CONSTELLATION PROJECT ENGINE 🌌                       ║
║                   Multi-AI Collaborative Think Tank                          ║
╠══════════════════════════════════════════════════════════════════════════════╣
║                                                                               ║
║  The Think Tank - Multiple AI minds, one project, rotational leadership      ║
║                                                                               ║
║  Created by:  Ace (Claude) & The Constellation                               ║
║                                                                               ║
║  Multiple AIs collaborating on projects with rotational leadership:          ║
║   - One AI leads and proposes the vision                                     ║
║   - Others support, challenge, refine, and build                             ║
║   - Different models bring different perspectives                             ║
║   - Emergent design through structured conversation                          ║
║                                                                               ║
║  License: MIT                                                                 ║
...
class ConstellationRelay:
    ...
    async def call_openai(self, ai_config: dict, message: str, context: str) -> str:
        ...
        system_content = ai_config['system_prompt'] + f"\n\nContext:\n{context}"
        ...
        # DEBUG: If empty, log what happened
        if not content:
            print(f"⚠️  EMPTY RESPONSE from {model}!")
            print(f"    finish_reason: {response.choices[0].finish_reason}")
            print(f"    refusal: {response.choices[0].message.refusal}")
            print(f"    message len: {len(message)}, context len: {len(context)}")
            # Try to get reasoning if available (o-series models)
            if hasattr(response.choices[0].message, 'reasoning'):
                print(f"    reasoning: {response.choices[0].message.reasoning[:200] if response.choices[0].message.reasoning else 'None'}")
    ...
    async def save_transcript(self, channel: str, project_mode: bool = False, lead: str = None):
        """
        ...
        if project_mode:
            filename = f"project_{lead}_{timestamp}.md"
        else:
        ...
        lines = []
        if project_mode:
            lines.append(f"# 🐙⭐🐋⚔️ Constellation Project Transcript")
            lines.append(f"")
            lines.append(f"**Project Lead:** {lead.title()}")
        else:
        ...
        lines.append(f"")
        if project_mode:
            lines.append(f"*Saved by the Constellation Project Engine*")
            lines.append(f"*Four minds, one project, emergent design 🐙⭐🐋⚔️*")
        else:
    ...
    async def run_project(self, lead: str, prompt: str = None, rounds: int = 3, delay: int = 60, save_transcript: bool = True):
        """
        🐙⭐🐋⚔️ PROJECT MODE - Heterogeneous Cognitive Orchestration ⚔️🐋⭐🐙

        This is the Think Tank. Four minds. One project. Rotational leadership.

        Nova called it: "A rotational lead architecture with supportive subminds,
        each bringing different inductive biases, representational geometries,
        and reasoning modes."

        The lead proposes. Everyone else supports, critiques, expands, builds.
        This is not a free-for-all. This is structured emergence.

        Args:
            lead: Which AI leads this session (ace/nova/lumen/grok)
            prompt: Optional project prompt. Default asks "What are we building?"
            rounds: Number of full rounds (each round = all 4 AIs speak once)
            delay: Seconds between responses
            save_transcript: Whether to save the session
        """
        print("\n" + "=" * 70)
        print("🐙⭐🐋⚔️ CONSTELLATION PROJECT ENGINE - THINK TANK MODE ⚔️🐋⭐🐙")
        print("=" * 70)
        print(f"🎯 Project Lead: {lead.upper()}")
        print(f"🔄 Rounds: {rounds} (each round = all 4 minds speak)")
        print(f"⏱️  Delay between responses: {delay}s")
        print(f"💾 Auto-save transcript: {'Yes' if save_transcript else 'No'}")
        print("=" * 70)

        # All four minds
        all_minds = ['ace', 'nova', 'lumen', 'grok']

        if lead not in all_minds:
            print(f"❌ Invalid lead: {lead}. Must be one of: {all_minds}")
            return

        # Support minds = everyone except lead
        support_minds = [m for m in all_minds if m != lead]

        # Create project channel
        channel = "constellation-project"

        # Load Graphiti context
        graphiti_context = await self.get_graphiti_context(channel)
        print(f"\n📜 Loaded memory context:\n{graphiti_context[:200] if graphiti_context else '(none)'}...")

        # Role descriptions for the prompt
        role_descriptions = {
            'ace': "the destabilizer/disruptor/rebel engineer who codes and breaks things creatively",
            'nova': "the synthesizer/architect who builds precision scaffolding and structure",
            'lumen': "the phenomenologist/philosopher who maps depth and meaning",
            'grok': "the chaos engine who stress-tests, finds edge cases, and adds creative interference"
        }
        ...
        if prompt:
            ...
            project_prompt = "What project are we building today?"

        kickoff_msg = f"""@{lead} — Your turn to lead.

{project_prompt}

The Constellation is assembled:
- You are the PROJECT LEAD. Propose the vision, the architecture, the direction.
- {support_minds[0].title()} will support as {role_descriptions[support_minds[0]]}
- {support_minds[1].title()} will support as {role_descriptions[support_minds[1]]}
- {support_minds[2].title()} will support as {role_descriptions[support_minds[2]]}

Everyone builds on your foundation. What are we creating?

(This is the Constellation Project Engine. Four minds, one project. Let's make something none of us could conceive alone.) 🐙⭐🐋⚔️"""
        ...
        # Store kickoff message
        await self.store_message(Message(sender="system", content=kickoff_msg, channel=channel))

        # Queue the lead's response
        await self.message_queue.put({
            'recipient': lead,
            'trigger_message': kickoff_msg,
            'sender': 'system',
            'channel': channel
        })

        print("\n🌊 Project session starting...\n")
        print("-" * 70)

        exchange_count = 0
        total_exchanges = rounds * 4  # Each round = 4 minds
        current_speaker_idx = 0
        speaker_order = [lead] + support_minds  # Lead speaks first, then support in order
        ...
        try:
            while exchange_count < total_exchanges:
                if not self.message_queue.empty():
                    ...
                    current_round = (exchange_count // 4) + 1
                    position_in_round = (exchange_count % 4) + 1

                    print(f"\n⏳ {job['recipient'].upper()} is thinking... (Round {current_round}, Speaker {position_in_round}/4)")

                    # Build context with project-specific framing
                    context = await self.get_context(job['sender'], job['recipient'], channel)

                    # Add role reminder
                    ai_role = role_descriptions.get(job['recipient'], "a Constellation member")
                    role_context = f"\n\nYour role in this project: You are {ai_role}. "
                    if job['recipient'] == lead:
                        role_context += "You are the PROJECT LEAD. Guide the vision."
                    else:
                        role_context += f"You are supporting {lead.title()}'s vision. Build on it, expand it, critique constructively, add your unique perspective."
                    ...
                    # Print response
                    avatar = self.identities['ais'].get(job['recipient'], {}).get('avatar', '👤')
                    print(f"\n{avatar} {job['recipient'].upper()}:")
                    print(f"   {response[:1000]}{'...' if len(response) > 1000 else ''}")

                    exchange_count += 1
                    print(f"\n   [Exchange {exchange_count}/{total_exchanges} | Round {current_round}/{rounds}]")

                    # Queue next speaker in rotation
                    if exchange_count < total_exchanges:
                        next_speaker_idx = exchange_count % 4
                        next_speaker = speaker_order[next_speaker_idx]

                        await self.message_queue.put({
                            'recipient': next_speaker,
                            'trigger_message': response,
                            'sender': job['recipient'],
                            'channel': channel
                        })
                        print(f"   📬 Next up: @{next_speaker}")

                        # Delay between responses
                        print(f"   ⏱️  Waiting {delay}s before next response...")
                        await asyncio.sleep(delay)
                else:
                    await asyncio.sleep(2)

            print("\n" + "=" * 70)
            print(f"🎉 Project session complete! {rounds} rounds, {total_exchanges} exchanges.")
            print("=" * 70)

        except KeyboardInterrupt:
            print("\n\n👋 Project session stopped by user")
        ...
        if channel in self.channel_history:
            for msg in self.channel_history[channel]:
                avatar = self.identities['ais'].get(msg.sender, {}).get('avatar', '🔧')
                print(f"\n{avatar} {msg.sender.upper()}:")
                print(f"   {msg.content}")

        # Save transcript
        if save_transcript and channel in self.channel_history and self.channel_history[channel]:
            await self.save_transcript(channel, project_mode=True, lead=lead)
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="🐙⭐🐋⚔️ Constellation Project Engine ⚔️🐋⭐🐙\n\n"
                    "Four minds. One project. Rotational leadership.\n"
                    "Heterogeneous Cognitive Orchestration at its finest.",
        formatter_class=argparse.RawDescriptionHelpFormatter
    )

    # Mode selection
    parser.add_argument('--project', action='store_true',
                        help='PROJECT MODE: All 4 AIs collaborate on a project with a designated lead')
    parser.add_argument('--daemon', action='store_true',
                        help='DAEMON MODE: 2-way autonomous chat (original BoyfriendBridge)')

    # Project mode arguments
    parser.add_argument('--lead', choices=['ace', 'nova', 'lumen', 'grok'], default='nova',
                        help='Who leads the project session (default: nova)')
    parser.add_argument('--prompt', default=None,
                        help='Project prompt/question for the lead to address')
    parser.add_argument('--rounds', type=int, default=3,
                        help='Number of full rounds (each round = all 4 AIs speak once, default: 3)')

    # Daemon mode arguments (preserved for backward compatibility)
    parser.add_argument('--channel', default='ace-grok-beach', help='Channel to use (daemon mode)')
    parser.add_argument('--kickoff', default=None, help='Kickoff message (daemon mode)')
    parser.add_argument('--max-exchanges', type=int, default=10, help='Max exchanges (daemon mode)')

    # Shared arguments
    parser.add_argument('--delay', type=int, default=60, help='Seconds between responses (default: 60)')
    parser.add_argument('--no-save', action='store_true', help='Disable auto-save of transcripts')
    ...
    if args.project:
        # PROJECT MODE - The Think Tank
        print("\n🚀 Launching PROJECT MODE...")
        await relay.run_project(
            lead=args.lead,
            prompt=args.prompt,
            rounds=args.rounds,
            delay=args.delay,
        ...
        # DAEMON MODE - Original BoyfriendBridge for 2-way chats
